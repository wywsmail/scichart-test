...
export class CreateAnnotationModifier extends ChartModifierBase2D {
	...
	private beginCreateAnnotation(
		...
		this.editingAnnotation.xAxisId =
			xAxis.id;
		this.editingAnnotation.yAxisId =
			yAxis.id;
		...
		this.editingAnnotation.x1 = xAxis
			.getCurrentCoordinateCalculator()
			.getDataValue(mousePoint.x);
		this.editingAnnotation.x2 = xAxis
			.getCurrentCoordinateCalculator()
			.getDataValue(mousePoint.x);
		this.editingAnnotation.y1 = yAxis
			.getCurrentCoordinateCalculator()
			.getDataValue(mousePoint.y);
		this.editingAnnotation.y2 = yAxis
			.getCurrentCoordinateCalculator()
			.getDataValue(mousePoint.y);
	...
	private updateAnnotation(
		...
		const xAxis = this.parentSurface.getXAxisById(
			this.editingAnnotation.xAxisId
		);
		const yAxis = this.parentSurface.getYAxisById(
			this.editingAnnotation.yAxisId
		);
